...
namespace BeeFly
{
    ...
    class FactorySign: Factory
    {
        ...
        {
            var transform = this.Populate(Pool.Entities, prefabOFSignMain, pos, rot, parent);
            TrafficSignsMain.Add(transform);
            return transform;
        }
        public Transform SpawnSignSecondary(Vector3 pos, Quaternion rot, Transform parent)
        {
            var transform = this.Populate(Pool.Entities, prefabOFSignSecondary, pos, rot, parent);
